...
export function Contato() {
  ...
  const { eyebrow, heading, intro, whatsappCtaLabel, whatsappMessage } = home.contato
  const serviceOptions = [...home.servicos.items.map(s => s.name), 'Outro']
    ...
    >
      <div className="site-container">
        ...
        <motion.div
          ...
          className="mb-16"
        >
          <p className="font-mono-mm text-[11px] tracking-[0.14em] mb-4" style={{ color: '#C9A84C' }}>{eyebrow}</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[40%_52%] gap-14 lg:gap-20">
          ...
          >
            ...
            <div className="flex flex-col gap-4">
              {[
                { icon: Phone,  href: whatsappLink(settings.whatsapp),                                text: settings.whatsapp },
                { icon: Mail,   href: `mailto:${settings.email}`,                                      text: settings.email },
                { icon: AtSign, href: `https://instagram.com/${settings.instagram.replace('@', '')}`,  text: settings.instagram },
                { icon: MapPin, href: '#',                                                              text: `${settings.cidade} — Brasil` },
          ...
          >
            {sent ? (
              ...
              <form onSubmit={handleSubmit} className="flex flex-col gap-7">
                ...
                <FloatingSelect name="servico" label="Tipo de serviço"     value={form.servico} onChange={handleChange} options={serviceOptions} />
