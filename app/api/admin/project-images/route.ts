...
/**
 * POST — upload de imagem do projeto (capa ou galeria).
 * FormData: projectId, file. Retorna { id, url, key }.
 */
export async function POST(request: Request) {
  ...
  try {
    ...
    const objectKey = projectImageObjectKey(projectId, filename)
    ...
    return NextResponse.json({
      id: `img-${Date.now()}`,
      url: projectImagePublicUrl(objectKey),
      key: objectKey,
    })
  } catch (error) {
...
/**
 * DELETE — remove imagem do projeto.
 * Query: ?projectId=<id>&key=projects/<id>/arquivo.jpg
 */
export async function DELETE(request: Request) {
  ...
  try {
    ...
    const expectedPrefix = `${PROJECT_IMAGE_PREFIX}${projectId}/`
